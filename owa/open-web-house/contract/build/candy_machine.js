...
class Item {
  constructor() {
    this.dulce, this.cantidad, this.precio;
  }
}

var _dec, _dec2, _dec3, _class, _class2;
let CandyMachine = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec(_class = (_class2 = class CandyMachine {
  constructor() {
  ...
  rellenarDulces({
    ...
    log(`Rellenando dulce: ${dulce}, cantidad: ${cantidad}, precio: ${precio},`);
    let empleado = predecessorAccountId();
...
}, (_applyDecoratedDescriptor(_class2.prototype, "rellenarDulces", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "rellenarDulces"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "obtenerDulces", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "obtenerDulces"), _class2.prototype)), _class2)) || _class);
function obtenerDulces() {
  const _state = CandyMachine._getState();
  if (!_state && CandyMachine._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = CandyMachine._create();
  if (_state) {
    CandyMachine._reconstruct(_contract, _state);
  }
  const _args = CandyMachine._getArgs();
  const _result = _contract.obtenerDulces(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CandyMachine._serialize(_result, true));
}
function rellenarDulces() {
  const _state = CandyMachine._getState();
  if (!_state && CandyMachine._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = CandyMachine._getArgs();
  const _result = _contract.rellenarDulces(_args);
  CandyMachine._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CandyMachine._serialize(_result, true));
}

export { CandyMachine, obtenerDulces, rellenarDulces };
//# sourceMappingURL=candy_machine.js.map
